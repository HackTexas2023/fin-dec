...
c(f.name,e).focus();c(f.name,e).addClass(f.errorClass);c("label[for="+c(f.name).attr("id")+"]",e).addClass(f.errorClass)}a.drawInvalid&&e.prepend(['<p class="',f.errorClass,'">',f.errorMessageDraw,"</p>"].join(""))}function w(){if(b.notValidateForm){return true;} var a=true,e={drawInvalid:false,nameInvalid:false},f=[d,b],g=[e,d,b];b.onBeforeValidate&&typeof b.onBeforeValidate==="function"?b.onBeforeValidate.apply(q,f):E.apply(q,f);if(b.drawOnly&&l.length<1){e.drawInvalid=true;a=false}if(c(b.name,d).val()===""){e.nameInvalid=true;a=
 ...
 * @author  mustafa
 *
 * necessary code piece for jotform
 */
// document.observe('dom:loaded', function(){  
//     jQuery(JotForm.forms[0]).submit(function(e){
//         e.preventDefault();
//         if(!window.sigPad.validateForm()){
//             return;
//         }
//         jQuery("input#sigPng").val(window.sigPad.getSignatureImage());

//         if(JotForm.validateAll() && window.sigPad.validateForm()){
//             JotForm.forms[0].submit();
//         }
//     });
// });
